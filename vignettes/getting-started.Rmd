...
- `ward.D`, called also MISSQ (Minimum Increase of Sum of SQuares): $d(A \cup B, C) = \frac{\left | A \cup B \right | \cdot \left | C \right |}{\left | A \cup B \cup C \right |} \cdot \left \| \frac{ \sum_{x \in A \cup B} x}{\left | A \cup B \right |} -
                                        \frac{ \sum_{y \in C} y}{\left | C \right |}
                                ...
                                $d(A \cup B, C) = \frac{\left | A \cup B \right | \cdot \left | C \right |}{\left | A \cup B \cup C \right |} \cdot  d \left( \frac{ \sum_{x \in A \cup B} x}{\left | A \cup B \right |},
                                        \frac{ \sum_{y \in C} y}{\left | C \right |}
                                \right ) ^2$.
...
- `ward.D2`: added to `stats::hclust` in `R >3.0.3` versions to implement the original *Ward's* linkage function [Murtagh and Legendre, 2014] which is *not* implemented with `ward.D`. The reported height in `ward.D2` is the square root of the height in `ward.D`, i.e. $d(A \cup B, C) = \sqrt{\frac{\left | A \cup B \right | \cdot \left | C \right |}{\left | A \cup B \cup C \right |} } \cdot \left \| \frac{ \sum_{x \in A \cup B} x}{\left | A \cup B \right |} -
                                ...
                                $d(A \cup B, C) = \sqrt{ \frac{\left | A \cup B \right | \cdot \left | C \right |}{\left | A \cup B \cup C \right |} } \cdot  d \left( \frac{ \sum_{x \in A \cup B} x }{\left | A \cup B \right |},
                                        \frac{ \sum_{y \in C} y }{\left | C \right |}
                                \right )$.
