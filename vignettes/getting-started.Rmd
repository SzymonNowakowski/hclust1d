...
If you want to provide the `dist` structure, please remember to change a `distance` argument to `TRUE` in a call to `hclust1d`. The two examples below return results that are completely equivalent. 
...
The three examples above return results that are completely equivalent (up to the order and shift, because again you will notice, that the first clustering is a mirror reflection of the second and the third clusterings). 
