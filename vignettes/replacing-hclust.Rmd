...
4. The object returned from `returned `hclast1d` call is the same S3 class as the result of `stats::hclust`, namely `hclust` S3 class.
...
You can see the list of all supported linkage functions by running
```{r supported_methods}
supported_methods()
```

The in-depth description of the linkage functions in `hclust1d`, together with the inter-cluster distance metric definition used in case of each linkage function (and returned as the height) can be found in our [getting started vignette](https://cran.r-project.org/package=hclust1d/vignettes/getting-started.html).
...
## `ward.D`, `centroid` or `median` linkage functions
...
Somewhere below in the code `res` gets analysed, but it is OK, because the results of both calls are compatible.
...
## All other linkage functions, beside `ward.D`, `centroid` or `median`
